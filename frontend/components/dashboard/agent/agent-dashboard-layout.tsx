...
const AGENT_NAV = [
  {
    ...
    match: (path: string) =>
      path === "/dashboard/agent" || path === "/dashboard/agent/",
  },
  {
    href: "/dashboard/agent/scan",
    label: "Scan QR",
    icon: ScanLine,
    match: (path: string) => path.startsWith("/dashboard/agent/scan"),
  },
] as const;
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent side="left" className="w-72 border-border/80 p-0">
                <SheetHeader className="flex flex-row items-center gap-2 border-b border-border/80 px-4 py-3 text-left">
                  ...
                  <SheetTitle className="text-base">Agent console</SheetTitle>
                </SheetHeader>
                <div className="p-3">
